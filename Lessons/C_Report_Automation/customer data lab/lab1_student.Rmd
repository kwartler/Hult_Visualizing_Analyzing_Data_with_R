...
# Not Run
#cuDF <- read.csv('https://raw.githubusercontent.com/kwartler/Hult_Visualizing_Analyzing_Data_with_R/main/Lessons/C_Report_Automation/customer%20data%20lab/grpN_10_fakeCustomerData.csv')
```
...
*We will use `dplyr` because its easier but you can use base-R's `aggregate`.*
...
-   Create a `ggplot` with data `customerByStateB`.
    ...
    -   Adjust the `theme` to rotate the x-axis labels 90 degrees. This requires a new layer `theme()` with the parameter `axis.text.x` equal to `element_text(angle = 90)` , and add the following labels/titles
